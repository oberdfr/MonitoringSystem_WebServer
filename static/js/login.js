...
function posso() {
    let pwd = getPwd();
    let user = getUser();
    let string = "http://192.168.11.202:5000/posso?user=" + user + "&pwd=" + pwd;
...
function checkSession() {
    fetch("http://192.168.11.202:5000/check_session", { credentials: 'include' })
        .then(response => response.json())
        .then(data => {
            if (data.status === "authenticated") {
                window.location.href = "http://192.168.11.202:5000/dashboard.html";
            }
        })
